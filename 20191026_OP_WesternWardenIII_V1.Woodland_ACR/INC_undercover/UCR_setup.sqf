...
_racProfFacCiv = 0.2;                     //(Number) Multiplies the effect of racial profiling. Lower this number to simulate more multicultural civilian population
_racProfFacEny = 0.2;                     //(Number) Multiplies the effect of racial profiling. Lower this number to simulate more multicultural enemy forces
...
_regDetectRadius = 3;                  //Default detection radius for regular troops (this will expand and contract based on weather, time of day, and how the undercover unit is acting - civilians within this radius will be under much more scrutinty)
...
_globalSuspicionModifier = 0.2;           //Scales the level of suspicion of enemies. 1 is default, 2 means units are twice as likely to see through undercover unit's disguises, 0.5 means half as likely etc.
...
_civilianVehicleArray = ["B_LSV_01_unarmed_F","B_Quadbike_01_F","B_T_UGV_01_rcws_olive_F","C_Offroad_01_covered_F"];
...
_incogFactions = ["I_L_F","I_C_F"]; //Array of enemy factions whose items and vehicles will allow the player to impersonate the enemy
